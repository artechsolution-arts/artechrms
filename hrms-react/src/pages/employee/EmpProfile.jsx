...
import {
  ...
  GraduationCap, Clock, FileText, Pencil, Check, X, History,
  ArrowRight, TrendingUp, TrendingDown, RefreshCw, UserCheck,
          ...
          <div className="mt-3 space-y-2">
            ...
            {salaryChange && (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-gray-500 text-xs">₹{Number(event.salary_before).toLocaleString('en-IN')}</span>
                <ArrowRight size={12} className="text-gray-400" />
                <span className={`text-xs font-semibold ${event.salary_after > event.salary_before ? 'text-green-600' : 'text-red-500'}`}>
                  ₹{Number(event.salary_after).toLocaleString('en-IN')}
                </span>
              </div>
            )}
